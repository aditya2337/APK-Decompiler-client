...
export default class FileTree extends Component {
  ...
  handleFileClick (file) {
    axios.get(`http://52.15.193.198:3002/users/app/get-code?filePath=${file.path}`)
  ...
  render () {
    ...
    const { redirectToRefferer, fileData, selectedFile } = this.state;

    if (redirectToRefferer) {
      return (
        <Redirect to={{
          pathname: '/edit-file',
          state: {fileData, selectedFile}
        }} />
      );
    }

    return (
      <div id='container_id' className='container-fluid bd-mt'>
        <NestedFileTreeView
